    ...
    <div class="card-header-table">
        <div class="search-button-container">
            ...
            <div class="sort-filter-container">
                <button id="view-toggle" class="btns" type="button">
                    <span>มุมมองแบบกลุ่ม</span>
                ...
                @foreach ($evidences as $index => $evidence)
                    ...
                    <tr>
                        ...
                        <td class="text-xs sm:text-sm align-top w-1/4">
                            ...
                            @elseif($isOffice)
                                <a href="{{ route('evidences.download', $evidence->id) }}" target="_blank" rel="noopener noreferrer"
                                    class="block w-full text-left rounded p-2 hover:bg-gray-50">
                        ...
                        <td data-search="{{ optional($indicator)->code ?? '' }}" class="relative group w-1/4">
                            <span onclick="window.location='{{ $rowUrl }}';"
                                ...
                                class="text-xs text-pretty sm:text-sm text-gray-700 hover:text-red-600 cursor-pointer">{{ $indicator->name ?? '-' }}</span>
                            @php
                                $docStatus = $indicator ? ($indicatorDocStatus[$indicator->id] ?? 'รอดำเนินการ') : null;
                                $docClass = match ($docStatus) {
                                    'ครบ' => 'status-completed',
                                    'ไม่ครบ' => 'status-error',
                                    default => 'status-warning',
                        ...
                        <td class="text-center w-fit">
                            ...
                            @else
                                <button type="button"
                                    ...
                                    onclick="window.open('{{ route('evidences.download', $evidence->id) }}', '_blank')"
                                    title="ดาวน์โหลดไฟล์">
...
@push('styles')
    ...
    <style>
        ...
        .indicator-toggle.is-open .indicator-chevron {
            transform: rotate(180deg);
        }
        ...
        .criteria-evidence-row a {
            color: #2563eb;
            text-decoration: underline;
        }

        .criteria-evidence-empty {
            padding: 10px 12px;
            font-size: 12px;
            color: #6b7280;
            text-align: center;
